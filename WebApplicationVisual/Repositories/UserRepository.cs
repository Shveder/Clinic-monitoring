...
public class UserRepository : RepositoryBase 
{
    public void WriteNewUserToDatabase(string login, string password)
    {
        ...
        var user = new Models.User(1, login, password, 0, salt, 1,false, false);
        using var connection = new NpgsqlConnection(connectionString);
       
        var sql = "INSERT INTO dentist.users (login, password, balance, salt, role, is_deleted, is_blocked) " +
    ...
    {
        using var connection = new NpgsqlConnection(connectionString);
        var sql = "SELECT COUNT(*) FROM dentist.users WHERE login = @login";
        using var command = new NpgsqlCommand(sql, connection);
    ...
    public User Login(string login, string password)
    {
        ...
        User user;
        connection.Open();
        var sql = "SELECT * FROM dentist.users WHERE login = @login AND password = @password";
        using (var command = new NpgsqlCommand(sql, connection))
        {
            command.Parameters.AddWithValue("login", login);
    ...
    public string GetSaltByUserName(string name)
    {
        NpgsqlConnection connection = new NpgsqlConnection(connectionString);
        connection.Open();
        var sql = "SELECT salt FROM dentist.users WHERE login = @name";
        string salt = null;
        using (var command = new NpgsqlCommand(sql, connection))
        {
    ...
    public User GetUserById(int userId)
    {
        ...
        User user;
        connection.Open();
        var sql = "SELECT * FROM dentist.users WHERE id = @userId";
        using (var command = new NpgsqlCommand(sql, connection))
        {
    ...
    {
        using var connection = new NpgsqlConnection(connectionString);
        var sql = "UPDATE dentist.users SET login = @newLogin WHERE id = @id";
        using var command = new NpgsqlCommand(sql, connection);
    ...
    {
        using var connection = new NpgsqlConnection(connectionString);
        var sql = "UPDATE dentist.users SET password = @password WHERE id = @id";
        using var command = new NpgsqlCommand(sql, connection);
        command.Parameters.AddWithValue("@id", id);
        command.Parameters.AddWithValue("@password", password);
        OpenConnection(connection);
    ...
    {
        using var connection = new NpgsqlConnection(connectionString);
        var sql = "INSERT INTO dentist.recent_passwords  (password, user_id) VALUES (@password, @user_id)";
        using var command = new NpgsqlCommand(sql, connection);
    ...
    public List<RecentPasswords> GetUserPasswordsHistory(int userId)
    {
        ...
        connection.Open();
        var sql = "SELECT * FROM dentist.recent_passwords WHERE user_id = @user_id";
        using (var command = new NpgsqlCommand(sql, connection))
        {
            command.Parameters.AddWithValue("user_id", userId);
            using (var reader = command.ExecuteReader())
    ...
    public void ReplenishBalance(int id, double sumOfDep)
    {
        ...
        using var connection = new NpgsqlConnection(connectionString);
        var sql = "INSERT INTO dentist.deposits (date, sum_of_deposit, user_id) VALUES (@date, @sum, @user_id)";
        using var command = new NpgsqlCommand(sql, connection);
